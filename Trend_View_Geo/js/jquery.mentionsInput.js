    ...
    var MentionsInput = function (settings) {
        ...
        function populateDropdown(query, results) {
            ...
            _.each(results, function (item, index) {
                ...
                autocompleteItemCollection[itemUid] = _.extend({}, item, {value: item.email,email:item.email,name:item.name}); //Inserts the new item to autocompleteItemCollection

                var elmListItem = $(settings.templates.autocompleteListItem({
debugger;
                    'id'      : utils.htmlEncode(item.email)
                 ...
                 var elmIcon;
                                    ...
                                    if (item.email) {
                                        elmIcon = $(settings.templates.autocompleteListItemAvatar({ email : item.email }));
                                    }
        ...
        function resetInput(currentVal) {
            ...
            while ((match = regex.exec(mentionText)) != null) {
                ...
                mentionsCollection.push({ 'id': match[4], 'email': match[2], 'type': match[3], 'value': match[2], 'trigger': match[1] });
            }
