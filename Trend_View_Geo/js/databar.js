...
(function ($) {
    var ColorMaker = function (options) {
        ...
        this.color = (function (self) {
            ...
            var colors = [
                'rgba(61, 132, 166, ' + backgroundOpacity + ')',   // '#D3D3D3',
                'rgba(61, 132, 166, ' + backgroundOpacity + ')',   // '#D3D3D3',
                'rgba(61, 132, 166, ' + backgroundOpacity + ')',   // '#D3D3D3',
                'rgba(61, 132, 166, ' + backgroundOpacity + ')',  // '#D3D3D3',
                'rgba(61, 132, 166, ' + backgroundOpacity + ')', // '#D3D3D3',
    ...
    $.fn.databar = function (options) {
        ...
        for (var i = 0; i < column_size; i++) {
            if(i == options.tdValue){
            var $vertical_tds = $table.find('tbody tr > :nth-child(' + (options.tdValue + 1) + ')');
            var numbers = $vertical_tds.map(function (i) {
                var text = $(this).attr('name');

                var stripped = text.replace(/[\s,%$円€\\]/g, '');
                if ($.isNumeric(stripped) && stripped > -1) {
            ...
            (function ($tds, options) {
                ...
                $tds.each(function (i) {
                    ...
                    var $bar = $('<span />').addClass($table[0].id+'Style')
                        .css($.extend({
                            'position': 'absolute',
                            'top': 0,
                            'left': 0,
